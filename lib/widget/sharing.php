...
if ( ! class_exists( 'WpssoSsbWidgetSharing' ) && class_exists( 'WP_Widget' ) ) {
	...
	class WpssoSsbWidgetSharing extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( is_object( $this->p->ssb ) && 
				$this->p->ssb->is_post_buttons_disabled() ) {
		...
		public function update( $new_instance, $old_instance ) {
			...
			foreach ( $this->p->ssb->get_defined_website_names() as $id => $name )
		...
		public function form( $instance ) {
			...
			foreach ( $this->p->ssb->get_defined_website_names() as $id => $name ) {
				...
				switch ( $id ) {
					case 'pinterest':
						echo ' '._x( '(not added on indexes)', 'option comment', 'wpsso-ssb' );
						break;
					case 'tumblr':
						echo ' '._x( '(shares link on indexes)', 'option comment', 'wpsso-ssb' );
						break;
				}
				echo '</label></p>', "\n";
			}
