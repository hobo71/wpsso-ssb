...
if ( ! function_exists( 'wpssossb_get_sharing_buttons' ) ) {
	function wpssossb_get_sharing_buttons( $ids = array(), $atts = array() ) {
		...
		if ( $wpsso->is_avail['ssb'] ) {
			...
			if ( ! defined( 'WPSSOSSB_BUTTONS_CACHE_EXP' ) )
				define( 'WPSSOSSB_BUTTONS_CACHE_EXP', 
			...
			if ( $wpsso->is_avail['cache']['transient'] ||
				...
				if ( $wpsso->is_avail['cache']['transient'] )
					set_transient( $cache_id, $html, WPSSOSSB_BUTTONS_CACHE_EXP );
				elseif ( $wpsso->is_avail['cache']['object'] )
					wp_cache_set( $cache_id, $html, __METHOD__, WPSSOSSB_BUTTONS_CACHE_EXP );

				if ( $wpsso->debug->enabled )
					$wpsso->debug->log( $cache_type.': html saved to cache '.
						$cache_id.' ('.WPSSOSSB_BUTTONS_CACHE_EXP.' seconds)');
			}
