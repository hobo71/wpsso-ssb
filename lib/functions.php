...
if ( ! function_exists( 'wpssossb_get_sharing_buttons' ) ) {
	function wpssossb_get_sharing_buttons( $ids = array(), $atts = array(), $cache_exp_secs = false ) {
		...
		$cache_index    = $wpsso->ssb_sharing->get_buttons_cache_index( $type, $atts, $ids );	// returns salt with locale, mobile, wp_query, etc.
		$cache_array    = array();
		...
		$cache_array[$cache_index] = $wpsso->ssb_sharing->get_html( $ids, $atts, $mod );
