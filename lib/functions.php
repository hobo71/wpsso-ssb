...
if ( ! function_exists( 'wpssossb_get_sharing_buttons' ) ) {
	function wpssossb_get_sharing_buttons( $ids = array(), $atts = array(), $cache_exp_secs = false ) {
		...
		$cache_exp_secs = $cache_exp_secs === false ? $wpsso->ssb_sharing->get_buttons_cache_exp() : $cache_exp_secs;
		...
		if ( $cache_exp_secs > 0 ) {
			...
			$cache_index = $wpsso->ssb_sharing->get_buttons_cache_index( $type, $atts, $ids );
		...
		if ( ! isset( $buttons_array[$cache_index] ) ) {
			...
			$buttons_array[$cache_index] = $wpsso->ssb_sharing->get_html( $ids, $atts, $mod );
...
'<!-- '.$lca.' '.__FUNCTION__.' function end -->'."\n\n";
				...
				if ( $cache_exp_secs > 0 ) {
					...
					if ( $wpsso->debug->enabled ) {
						$wpsso->debug->log( $type.' buttons html saved to transient cache (expires in '.$expires_in_secs.' seconds)' );
					}
				}
			}
		}
