...
if ( ! class_exists( 'WpssoSsbSubmenuShareGplus' ) ) {

	class WpssoSsbSubmenuShareGplus {
		...
		public function filter_ssb_share_gplus_rows( $table_rows, $form, $submenu ) {
			...
			'<td>'.$form->get_select( 'gp_order', range( 1, count( $submenu->share ) ) ).'</td>';

			if ( $this->p->avail['*']['vary_ua'] ) {
				$table_rows[] = $form->get_tr_hide( 'basic', 'gp_platform' ).
			...
			$table_rows[] = $form->get_tr_hide( 'basic', 'gp_script_loc' ).
...
if ( ! class_exists( 'WpssoSsbShareGplus' ) ) {

	class WpssoSsbShareGplus {
