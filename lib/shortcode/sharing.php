...
if ( ! class_exists( 'WpssoSsbShortcodeSharing' ) ) {
	...
	class WpssoSsbShortcodeSharing {
		...
		public function add() {
			if ( ! empty( $this->p->options['plugin_shortcodes'] ) ) {
        			add_shortcode( WPSSOSSB_SHARING_SHORTCODE_NAME, array( &$this, 'shortcode' ) );
		...
		public function shortcode( $atts, $content = null ) { 
			...
			$lca = $this->p->cf['lca'];
			$atts = apply_filters( $lca.'_sharing_shortcode_'.WPSSOSSB_SHARING_SHORTCODE_NAME, $atts, $content );
			...
			if ( ! isset( $buttons_array[$buttons_index] ) ) {
				...
				$buttons_array[$buttons_index] = $this->p->ssb_sharing->get_html( $ids, $atts, $mod );
